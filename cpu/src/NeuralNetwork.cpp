...
Network::Network(vector<size_t> &l, vector<string> &activ_func) : layer_sizes(l), activation_func(activ_func){
	...
	this->layer_outputs = vector<vector<float>>(layer_sizes.size());  	
	for(int i=0;i < l.size(); ++i){
		layer_outputs[i].resize(l[i]);
	}
	this->layer_deltas = vector<vector<float>>(layer_sizes.size() - 1);
	for(int i=1;i < l.size(); ++i){
		layer_deltas[i-1].resize(l[i]);
	}
	...
	vector<string> activation_functions_  = {"sigmoid", "ReLU", "Leaky", "Linear"};
	for(const auto func : activation_func){
		activation_func_id.push_back(distance(activation_functions_.begin(), find(activation_functions_.begin(), activation_functions_.end(), func)));
	}

	activation_func_map.push_back(&sigmoid);
	activation_func_map.push_back(&ReLU);
	activation_func_map.push_back(&Leaky);
	activation_func_map.push_back(&Linear);
...
vector<float> Network::predict(const vector<float> &input){
	...
	for(size_t layer_index=1; layer_index < layer_sizes.size(); ++layer_index){
		...
		for(size_t i=0;i<current_layer_size;++i){
			...
			layer_outputs[layer_index][i] = activation_func_map[activation_func_id[layer_index-1]](weighted_sum); //saving to layer_outputs
			//layer_outputs[layer_index].push_back(activation_func_map[activation_func_id[layer_index-1]](weighted_sum)); //saving to layer_outputs
		}
	}
...
void Network::fit(const vector<float> &input, const vector<float> &true_output, const float learning_rate,const float grad_decay, int verbose, int optimize){
	...
	for(size_t i=0;i < layer_sizes.back();++i){
		...
		layer_deltas[layer_sizes.size() - 2][i] = (true_output[i]-layer_outputs.back()[i])*Fdash(layer_outputs.back()[i], activation_func.back());
	}
	...
	if(optimize){
		...
		for(size_t layer_index=layer_sizes.size() - 2;layer_index > 0 ;--layer_index){
			...
			//adding the delta of the first node
			//layer_deltas[layer_index-1].push_back(layer_delta_sum*Fdash(layer_outputs[layer_index][0], "ReLU"));
			layer_deltas[layer_index-1][0] = layer_delta_sum*Fdash(layer_outputs[layer_index][0], "ReLU");
			...
			for(size_t i=1;i < current_layer_size; ++i){ //iterate through each node in the layer
				...
				layer_deltas[layer_index-1][i] = layer_delta_sum*Fdash(layer_outputs[layer_index][i], activation_func[layer_index-1]);
			}
		}
		...
		for(size_t layer_index=layer_sizes.size() - 2;layer_index > 0 ;--layer_index){
			...
			layer_deltas[layer_index-1][0] = layer_delta_sum*Fdash(layer_outputs[layer_index][0], activation_func[layer_index-1]);
...
void Network::print_weights(ostream &out){
	out << "weights"<<'\n';
	for(int in = 0;in<layer_sizes.size()-1;in++){
		out << "layer "<<in<< '\n';
		for(auto weight: this->weights[in]){
			out << weight << ' ';
		}
		out << '\n';
	}
