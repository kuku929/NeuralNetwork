...
__global__ void initialize_gradient_(float *grad_bias, float *grad_weights,
                                     int M)
{
    // grad weights has M
    // columns
...
__global__ void find_delta_and_transpose(float *a, float *b, float *output,
                                         int N, int M)
{
    ...
    {
        output[transpose_index_in_vector] =
            a[index_in_vector] - b[index_in_vector];
...
__global__ void dev_vec_matmul(const float *dev_a, const float *dev_b,
                               float *dev_output, int N, int M)
{
    // a is NxM
    int ROW = threadIdx.x;
    float temp_value = 0;
...
__global__ void gemm(const float *a, const float *b, const float *c,
                     float *output, int N, int M, int K)
{
    ...
    if (row_in_matrix < N && col_in_matrix < M)
    {
        ...
        output[(row_in_matrix)*M + col_in_matrix] =
            temp_value + c[row_in_matrix];
    }
}

__global__ void matmul(const float *a, const float *b, float *output, int N,
                       int M, int K)
{
    // a is NxK
    // b is KxM
...
__global__ void update_bias(float *dev_bias, float *layer_delta, int N, int M,
                            float learning_rate)
{
    /*
...
__global__ void update_weights(float *dev_weights, float *layer_output,
                               float *layer_delta, int N, int M, int K,
                               float learning_rate)
{
    int bx = blockIdx.x, by = blockIdx.y;
    int row = threadIdx.y, col = threadIdx.x;
    ...
    if (row_in_matrix < N && col_in_matrix < M)
    {
        ...
        for (int i = 0; i < K; i += BLOCK_SIZE)
        {
            ...
            for (int j = 0; j < BLOCK_SIZE; ++j)
            {
                for (int k = 0; k < BLOCK_SIZE; ++k)
                {
                    ...
                    {
                        A[j][k] =
                            layer_output[(by * BLOCK_SIZE + j) * K + i + k];
                    }
                    else
                    {
                        A[j][k] = 0;
                    ...
                    if (i + j < K && bx * BLOCK_SIZE + k < M)
                    {
                        B[j][k] = layer_delta[(i + j) * M + bx * BLOCK_SIZE +
                                              k]; // bx*BLOCK_SIZE+k ->
                                                  // column, i+j -> row
                    }
                    else
        ...
        dev_weights[col_in_matrix * N + row_in_matrix] +=
            learning_rate * temp_value / K;
    }
}

__global__ void rmsprop_update_bias_(float *dev_bias, float *layer_delta,
                                     float *gradient_sum, int N, int M,
                                     float learning_rate, float beta)
{
    ...
    // //debug
    // if(COL == 0){
    // 	printf("rmsprop_bias_kernel
    // : %f\n", layer_delta[0]);
    // }

    gradient_sum[COL] =
        gradient_sum[COL] * beta + (1 - beta) * temp_value * temp_value;
    dev_bias[COL] +=
        learning_rate * temp_value / (1e-5 + sqrt(gradient_sum[COL]));
}

__global__ void rmsprop_update_weights_(float *dev_weights, float *layer_output,
                                        float *layer_delta, float *gradient_sum,
                                        int N, int M, int K,
                                        float learning_rate, float beta)
{
    /*
    ...
    if (row_in_matrix < N && col_in_matrix < M)
    {
        ...
        for (int i = 0; i < K; i += BLOCK_SIZE)
        {
            ...
            for (int j = 0; j < BLOCK_SIZE; ++j)
            {
                for (int k = 0; k < BLOCK_SIZE; ++k)
                {
                    ...
                    {
                        B[j][k] =
                            layer_delta[(i + j) * M + bx * BLOCK_SIZE + k];
                    }
                    else
                    {
                        B[j][k] = 0;
        ...
        gradient_sum[transposed_ind] = beta * gradient_sum[transposed_ind] +
                                       (1 - beta) * temp_value * temp_value;
        dev_weights[transposed_ind] +=
            learning_rate * temp_value /
            (1e-5 + sqrt(gradient_sum[transposed_ind]));
    }
}
